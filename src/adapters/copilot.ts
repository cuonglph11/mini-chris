...
export class CopilotAdapter implements Adapter {
  ...
  async *continueAfterToolCall(options: {
    systemPrompt: string;
    tools: ToolDefinition[];
    model?: string;
    cwd?: string;
    ...
    const parser = createParser({
      onEvent(sseEvent) {
        if (sseEvent.data === '[DONE]') { done = true; return; }
        let chunk: Record<string, unknown>;
        try { chunk = JSON.parse(sseEvent.data) as Record<string, unknown>; } catch { return; }
    ...
    for (const [, acc] of toolCallAccumulators) {
      let args: Record<string, unknown>;
      try { args = JSON.parse(acc.argumentsJson || '{}') as Record<string, unknown>; }
      catch { args = { _raw: acc.argumentsJson }; }
      yield { type: 'tool_call', id: acc.id, name: acc.name, args };
      toolCallsForHistory.push({
        id: acc.id,
        type: 'function',
